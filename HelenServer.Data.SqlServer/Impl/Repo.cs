...
{
    public class Repo<TContext> : IRepo<TContext>
        where TContext : DbContext
    {
        ...
        public Repo(TContext context)
        {
            ...
            CanWrite = context.Database.GetDbConnection().Database.Contains("master");
        }
        ...
        public IReadOnlyRepo<TContext> AsReadOnly()
        {
            CanWrite = false;

            return (IReadOnlyRepo<TContext>)this;
        }

        public IQueryable<TEntity> GetAll<TEntity>(bool noTracking = true)
            where TEntity : class, new()
        {
            return GetDbSet<TEntity>(noTracking);
        }

        public async Task<int> InsertAsync<TEntity>(TEntity entity, CancellationToken cancellationToken = default)
            where TEntity : class, new()
        {
        ...
        public async Task<int> InsertRangeAsync<TEntity>(IEnumerable<TEntity> entities, CancellationToken cancellationToken = default)
            where TEntity : class, new()
        {
        ...
        public async Task<int> RemoveAsync<TEntity>(TEntity entity, CancellationToken cancellationToken = default)
            where TEntity : class, new()
        {
        ...
        public async Task<int> RemoveRangeAsync<TEntity>(IEnumerable<TEntity> entities, CancellationToken cancellationToken = default)
            where TEntity : class, new()
        {
        ...
        public async Task<int> UpdateAsync<TEntity>(
            TEntity entity,
            Expression<Func<TEntity, object>>[] updatingExpressions,
        ...
        public async Task<bool> AnyAsync<TEntity>(Expression<Func<TEntity, bool>> whereExpression, CancellationToken cancellationToken = default)
            where TEntity : class, new()
        {
        ...
        public IQueryable<TEntity> Where<TEntity>(Expression<Func<TEntity, bool>> expression, bool noTracking = true)
            where TEntity : class, new()
        {
            return GetDbSet<TEntity>(noTracking).Where(expression);
        }

        public async Task<IPageModel<TEntity>> PageAsync<TEntity>(
            int pageIndex,
            int pageSize,
            Expression<Func<TEntity, bool>> whereExpression,
